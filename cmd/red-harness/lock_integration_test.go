...
//
// ⚠️ **两个进程显式指向同一条锁路径（`--lock`）**，而不是靠默认值——但理由是
// **换过一次**的，读的时候不要按旧理由理解。
//
// 写这条用例时，默认锁是 `<StoreDir>/run.lock`（每个 store 一把）。那正是 N0.2 要
// 修的「作用域算错」本身：两个不同 `--store` 的进程会拿到**两把**锁，于是靠默认值
// 根本制造不出争用，必须显式指定同一条路径。那时候「按 daemon 端点收敛」还是一条
// 在途的工作。
//
// **那条工作已经落地了**（`local/lock.go`）：默认锁是
// `/run/lock/red-harness/run-<daemon 端点指纹>.lock`，按「宿主 + daemon 端点」取，
// 所以今天这两个进程就算不传 `--lock` 也会落到同一条路径上。
//
// 仍然显式传，是为了**与宿主隔离**：默认路径是全宿主共享的，当另一个 worktree 的
// 集成门、或某个真在跑的 run 正占着它时，第一个进程会**取不到锁**——而这条用例会把
// 那次失败读成自己的红（一个「看起来像随机失败」的红，正是本仓库最贵的噪声）。
// 显式路径落在 `t.TempDir()` 里，争用只发生在本用例这两个进程之间，那才是它要证的事。
// 顺带：这条用例因此**不**证明「默认路径也是全宿主互斥」——默认路径的选取由
// `local/lock_test.go` 直接钉。
//
...
func TestIntegrationTwoStoresContendForTheRunLock(t *testing.T) {
	...
	// 不成立。**跨用户互斥并不发生**，但这是**机制的结果，不是哪里的明令**——这里原先
	// 写「跨用户互斥是明令不做的（见 `local/lock.go`）」，而那个文件里**没有**这句话，
	// 是一条指向空处的引用。真实原因有两条，都能在代码里读到：
	//   - 锁文件按 0600 创建（`local/lock.go` 的 `os.OpenFile`），所以第二个**用户**
	//     根本打不开它，失败在 KindPersistence「打开锁文件失败」——是报错，不是等待；
	//   - owner 身份含 uid（`harness.ResolveOwner`），于是同一宿主上不同用户的资源在
	//     回收判据里互为 owner_mismatch，只报告、不删。
	// 跨主机同理：远程 daemon 由 `harness.ResolveDockerEndpoint` 直接拒绝（宿主级锁
	// 本来就无法跨主机互斥）。锁的作用域是「**同一宿主 + 同一 daemon 端点**」。
