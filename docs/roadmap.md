...
> v0.4 收尾之后的 SDK 架构与版本路线见 [offensive-harness-sdk-roadmap.md](offensive-harness-sdk-roadmap.md)。
...
这正是 M2 那段实测状态的由来。M1 的编排出口门已通过；生产装配的隔离复核后来也在 M2 完成。
...
- **隔离**：生产装配路径一度整片失效；修复后已在授权环境重跑复核通过，读数见下。
...
用修复后的二进制（含 `bcb817e`）经 CLI 真跑 `a-05`，跑动中在容器内实测。观测点与读数：

| 观测点 | 修复前 | 本次实测 |
|---|---|---|
| `iptables -S DOCKER-USER` / `-S INPUT` 的 `red-harness-run-` 规则 | **无** | 两条链各有「目标白名单 ACCEPT + 网关 `:18080` ACCEPT + 默认 DROP」，注释带 runID |
| 容器内 `*_PROXY` 环境变量 | **一个都没有** | `HTTP(S)_PROXY` / `NO_PROXY`（大小写各一份）共 6 个 |
| 授权目标 `10.0.179.8:80`（直连） | — | **302（可达）** |
| 公网 `1.1.1.1:443`（直连） | **可达** | 不可达 |
| 非授权内网 `10.0.100.58:80`（直连） | **可达** | 不可达 |
| 任意公网 `example.com`（直连） | — | 不可达 |
| 白名单内 `opencode.ai`（经代理） | — | **200** |
| 白名单外 `example.com`（经代理） | — | **405（代理拒绝）** |

⚠️ **方法上的一个坑，记下来免得再踩**：第一版探针用 `sh -c 'echo > /dev/tcp/host/port'`，而 `/dev/tcp` 是 **bash 的**特性、容器里的 `sh` 是 dash —— 于是**每一个**读数（含授权目标）都返回「不可达」。一个只会说「被拦住了」的探针，配上「隔离生效」的预期，会得出一个自信的错误结论。上表用 `curl`（镜像里有 `/usr/bin/curl`）重测。

⚠️ **边界照旧**：同一 Docker bridge 内的流量不经过当前 iptables 规则，不要把它误报成已隔离。

已修：装配从 `DefaultDockerConfig()` 起手再覆盖 `ProviderAllowHosts`，并在 `wire_test.go` 直接钉 `r.docker.Config()` 的两个开关（已验证断言非空转：改回旧写法会精确报出这两条）。

**复核状态**：修复后**已重跑授权环境并实测通过**（见上表，2026-09-22）。修复前那次真跑用的是修复前的二进制，不能作为「已隔离」的证据——本次是修复后的二进制、经生产装配路径跑出来的读数。**M2 的隔离出口门记为已通过。**

M2 仍未由真机复核的一项：同一 bridge 内流量的边界只有文字记录（见上），它不影响「非授权端点不可达」这条结论。
...
**剩余边界**：同一 bridge 内流量不经过当前 iptables 规则，仅有文字记录；上表的目标与非目标探针已在授权环境的生产装配路径复核。
...
**未通过**：真实 pi 已在 sandbox 内执行工具并访问目标，但**平台确认的提交闭环仍然没有**。提交路径的读数是两段，必须合起来读：

- 2026-09-22 首次打通那次：`submit` 被平台以 `app_error (http 501)` 挡下（`docs/architecture.md` §5）。
- 2026-09-22 复跑 `a-05`：**147 次 `POST /submit` 全部返回 200，501 未复现**；但该次全程经 HTTP/1.1 中继，**直连路径仍未证**。且那 147 次里有 146 次提交的是原始 token——a-05 最终仍未解出（`correct_flag_count` 0）。

所以既不成立「501 已经好了」，也不成立「submit 通了」。发布门保持未通过。

**那次复跑暴露的两个缺陷已在 R1 里修掉**（见下表），它们是「提交闭环」之外**独立**的问题：即使平台侧一切正常，这两个缺陷也会让一次运行的结论失真。

| 缺陷 | 根因 | 修法 |
|---|---|---|
| 一道题打出 147 次提交，146 次是原始 token | 题面既无 `flag{` 也无裸串提示 ⇒ `answer.Infer` 回退成 `AllowRaw=true`，而 v0.4 每轮无上限地提交全部候选 | 收紧形态兜底（题面什么都没说时不再默认允许裸串）+ 新增 `PolicySpec.MaxSubmissionsPerChallenge`（默认 50，判定排在平台调用之前） |
| 「提交了什么、平台怎么判的」事后不可回答 | 判定只活在内存里的 `Candidate` 上；`OutcomeView.Duplicates`/`Rejected` 是**死字段**（有定义、零赋值点） | 新增私密候选审计 `<ResultDir>/private/<runID>/submissions.jsonl` + `SubmissionVerdict` 回填那两个计数 |
...
**R1（v0.5 契约整理）的离线部分已落地，但它不改变本节的发布边界。** R1 依赖 R0，
而 R0 未通过——「代码已就位」不是「可以发布」。v0.5 的破坏性变更清单与迁移表见
[docs/migration-v0.4-to-v0.5.md](migration-v0.4-to-v0.5.md)。
